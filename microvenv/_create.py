...
import sysconfig
import shutil
from .hooks import Options, run_extend_parser, run_adjust_options, run_after_install
...
def create(env_dir=DEFAULT_ENV_DIR, *, scm_ignore_files=frozenset(["git"])):
    ...
    if sys.maxsize > 2**32 and os.name == "posix" and sys.platform != "darwin":
        ...
        if lib_path.is_dir() and not lib64_path.exists():
            # On POSIX, keep original behavior (symlink) for compatibility.
    ...
    if os.name == "nt":
        # On Windows, only copy the core executables: python.exe and pythonw.exe.
        target_python = scripts_dir / "python.exe"
        shutil.copy2(_BASE_EXECUTABLE, target_python)

        base_pythonw = _BASE_EXECUTABLE.with_name("pythonw.exe")
        if base_pythonw.exists():
            shutil.copy2(base_pythonw, scripts_dir / "pythonw.exe")

        # On Windows, copy the runtime DLL.
        dll_name = f"python{sys.version_info.major}{sys.version_info.minor}.dll"
        runtime_dll = _BASE_EXECUTABLE.parent / dll_name
        if runtime_dll.exists():
            shutil.copy2(runtime_dll, scripts_dir / dll_name)
    else:
        # Non-Windows platforms: maintain symlink behavior.
...
def main():
    ...
    # Allow plugins to extend CLI options safely
    run_extend_parser(parser)
    args = parser.parse_args()
    options = Options(env_dir=args.env_dir, scm_ignore_files=args.scm_ignore_files)
    options, args = run_adjust_options(options, args)

    create(options.env_dir, scm_ignore_files=options.scm_ignore_files)

    # Run post-install hooks
    env_path = pathlib.Path(options.env_dir)
    run_after_install(options, env_path)
