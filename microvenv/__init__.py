...
# Lazily expose selected APIs to avoid importing submodules at package import
# time, which can trigger runpy warnings when executing submodules via
# `python -m microvenv._create`.
def __getattr__(name):
    if name in {"DEFAULT_ENV_DIR", "create"}:
        from . import _create as _create_mod
        return getattr(_create_mod, name)
    if name in {
        "register_extend_parser",
        "register_adjust_options",
        ...
        mapping = {
            "register_extend_parser": register_extend_parser,
            "register_adjust_options": register_adjust_options,
            "register_after_install": register_after_install,
        }
        return mapping[name]
