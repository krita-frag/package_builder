...
The CLI is available on all platforms, including Windows.
...
The code operates similarly to `py -m venv --without-pip .venv`,
except that:

- There are no activation scripts (you can execute `python` in the virtual environment directly)
- On Windows, executable shims are created via file copying instead of symlinks
